...
import { publishReplay, finalize, refCount } from 'rxjs/operators';
import { Http, getHttpWithResponseHeaders } from './http';
//import { API_HOST } from '../config';
export const API_HOST = process.env.NODE_ENV === 'production' ? '' : '/api';
...
export class ApiFetch {
    ...
    _buildQuery(url, query) {
        ...
        Object.keys(query).forEach(key => {
            ...
            if (key === '_') {
                logger.warn(
                    "query key should not equal to '_', it will be ignored"
                );
                return;
            }
